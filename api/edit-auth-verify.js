...
export default async function handler(req, res) {
  ...
  const { data: pending } = await supabase.from('auth_challenges').select('*').eq('challenge', challenge).single();
  if (!pending) return res.status(400).json({ error: 'Challenge expired or already used.' });
